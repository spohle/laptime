const LEGEND = [
  ...
  { key: 'unknown', label: 'No Schedule Match', swatch: 'bg-laneUnknown !text-laneUnknownFg' },
]
...
function LaneLegend() {
  return (
    <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-2 sm:mt-5 sm:gap-3">
      {LEGEND.map((item) => (
        <div key={item.key} className="flex min-w-0 items-center gap-2 text-[10px] text-uiBody sm:text-xs">
